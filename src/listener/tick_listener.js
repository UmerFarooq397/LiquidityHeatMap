...
module.exports = class TickListener {
  ...
  async startStrategyIntervals() {
    ...
    const types = [
      ...
      {
        name: 'trade',
        items: this.instances.symbols.filter(sym => sym.trade && sym.trade.strategies && sym.trade.strategies.length > 0)
      }
    ];
    ...
    types.forEach(type => {
      ...
      type.items.forEach(symbol => {
        ...
        strategies.forEach(strategy => {
          ...
          me.logger.info(
            `"${symbol.exchange}" - "${symbol.symbol}" - "${type.name}" - init strategy "${strategy.strategy}" (${myInterval}) in ${(
              timeoutWindow /
              60 /
              1000
            ).toFixed(3)} minutes`
          ...
          setTimeout(() => {
            me.logger.info(
              `"${symbol.exchange}" - "${symbol.symbol}" - "${type.name}" first strategy run "${strategy.strategy}" now every ${(interval / 60 / 1000).toFixed(
                2
              )} minutes`
            );
